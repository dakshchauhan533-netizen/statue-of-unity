            ...
            <div class="info">
                ...
                <?php
                    ...
                    if (isset($_GET['submit'])) {
                        $d1 = isset($_GET['date1']) ? $_GET['date1'] : null;
                        $d2 = isset($_GET['date2']) ? $_GET['date2'] : null;
                        $place = isset($_GET['place']) ? $_GET['place'] : null;

                        if (!empty($d1) && !empty($d2) && !empty($place)) {
                            $sql = "SELECT * FROM `ticket_book` WHERE place_name = '$place' AND `date` BETWEEN '$d1' AND '$d2'";
                            $data = mysqli_query($con, $sql);
                        } else {
                            $data = null; // Prevent SQL errors if no data is provided
                        }
                    }
                ?>
                <br>
